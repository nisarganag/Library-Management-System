...
public class FindConferences extends JFrame implements ActionListener{
    ...
    public void actionPerformed(ActionEvent e) {
        if(e.getSource()==bt1){
            ...
            try {
                ConnectionClass obj=new ConnectionClass();
                String q="SELECT * FROM conference WHERE CoAuthorInternal LIKE '%" + coAuthIn + "%'";
                ResultSet rest=obj.stm.executeQuery(q);
            while(rest.next()){
                y[i][j++]=rest.getString("Name");
                y[i][j++]=rest.getString("CorrespondingAuthor");
            ...
            try {
                ConnectionClass obj=new ConnectionClass();
                String q="select * from conference where PageNumber='"+pgno+"'";
                ResultSet rest=obj.stm.executeQuery(q);
            while(rest.next()){
                y[i][j++]=rest.getString("Name");
                y[i][j++]=rest.getString("CorrespondingAuthor");
            ...
            try {
                ConnectionClass obj=new ConnectionClass();
                String q="select * from conference where Date between '"+date1+"' and '"+date2+"'";
                ResultSet rest=obj.stm.executeQuery(q);
            while(rest.next()){
                y[i][j++]=rest.getString("Name");
                y[i][j++]=rest.getString("CorrespondingAuthor");
